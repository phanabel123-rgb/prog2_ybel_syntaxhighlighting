...
class MiniJavaTokensTest {
  ...
  private Token findTokenByColour(java.awt.Color colour) {
    return tokens.stream()
        ...
        .findFirst()
        .orElseThrow(
            () -> new AssertionError("Token mit der Farbe " + colour + " wurde nicht gefunden!"));
  }
  ...
  public void testPositions_BeginningMiddleEnd() {
    ...
    assertEquals(
        2,
        matches.size(),
        "Sollte das Keyword am Anfang und am Ende finden (class wird ignoriert, wenn nicht im"
            + " RegEx)");
  ...
  public void testEdgeCase_CommentContainsKeywords() {
    ...
    assertEquals(
        1,
        commentMatches.size(),
        "Der gesamte Zeilenkommentar muss als EIN Treffer erkannt werden.");

    List<HighlightRegion> keywordMatches = keywordToken.test(lineCommentText);
    assertFalse(
        keywordMatches.isEmpty(),
        "Das Keyword-Token sucht isoliert und findet die Wörter im Kommentar.");
  }
  ...
  public void testEdgeCase_Annotations() {
    ...
    assertEquals(
        ...
        "Die gesamte Annotation inkl. Bindestrich muss gematcht sein.");
  }
  ...
  public void testEdgeCase_StringsContainingCommentSymbols() {
    ...
    // Prüfen, ob der gesamte Inhalt inklusive der Kommentarzeichen als ein einziger String erkannt
    // wurde
